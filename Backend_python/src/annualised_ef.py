...
prices = yf.download(tickers, start="2022-06-01", end="2024-06-01")['Adj Close']
...
print(weights_record[results_frame['Sharpe'].idxmax()])
# Locate the portfolio with the minimum standard deviation
# min_vol_port = results_frame.iloc[results_frame['StdDev'].idxmin()]
# print(min_vol_port)
# print(weights_record[results_frame['StdDev'].idxmin()])

# Plotting the efficient frontier
plt.figure(figsize=(10, 7))
plt.scatter(results_frame.StdDev, results_frame.Return, c=results_frame.Sharpe, cmap='viridis')
plt.colorbar(label='Sharpe Ratio')
plt.scatter(max_sharpe_port[1], max_sharpe_port[0], color='red', marker='*', s=200, label='Max Sharpe Ratio')
# plt.scatter(min_vol_port[1], min_vol_port[0], color='blue', marker='*', s=200, label='Min Volatility')
